...
// capture needs Npcap, which needs cgo and a separate install whose licence
// forbids redistribution. Everything else the tool does — verify, decode,
// index, coverage — reads an archived session and needs none of that, so a
// plain `go build` produces a binary that can analyse bundles even though it
// cannot record one.
//
// Build with `-tags npcap` for a capture-capable binary.

const buildHint = "this build has no capture backend. Install Npcap " +
	"(https://npcap.com) and rebuild with: go build -tags npcap .\\cmd\\sccap"

func available() (bool, string) {
	return false, buildHint
}
